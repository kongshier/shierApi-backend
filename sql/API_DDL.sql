-- 创建库
create database if not exists shier_api;
...
-- 接口信息
create table if not exists shier_api.`interface_info`
(
...
insert into shier_api.`interface_info` (`name`, `description`, `url`, `requestHeader`, `responseHeader`, `status`,
                                        `method`, `userId`)
...
-- 用户接口关系表
create table if not exists shier_api.`user_interface_info`
(
    `id`              bigint                             not null auto_increment comment '主键' primary key,
